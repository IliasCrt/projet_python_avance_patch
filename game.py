...
class Game:
    ...
    def update(self,t0,t,screen):
        ...
        self.compteur+=1
        Lieux=["lieu1.png","lieu3.png","lieu4.png","lieu5.png","lieu6.png","lieu7.png"]
        if self.compteur%300==0 :
            random_index = random.randint(0,len(Lieux)-1)
            lieu=Lieux[random_index]
            self.background = pg.image.load(lieu)
        ...
        def obstacle(D):
            O = D
            p = rd.random()
            if p <= 0.004 or (len(D) == 0 and p <= 0.015):
                O.append([1200, rd.randint(120, 230), rd.randint(25, 85)])
            return O

        # Fonction de calcul de la vitesse des obstacles
        def v(t):
            if t - t0 < 10:
                v = 400
            else:
        ...
        for event in pg.event.get():
            if event.type == pg.QUIT:
                ...
                if event.key == pg.K_q:
                    running = False
            if (event.type == pg.KEYDOWN and event.key == pg.K_SPACE) and self.c_s < 2:
                self.c_s += 1
                self.z += -30
                self.dz = self.v0
        ...
        # Génération et défilement des obstacles
        self.obstacle = obstacle(self.obstacle)
        ...
        for o in self.obstacle :
            for k in range(10):
                x_a = 500 + self.r*math.cos((2*math.pi*k)/10)
                y_b = self.z-self.r + self.r*math.sin((2*math.pi*k)/10)
